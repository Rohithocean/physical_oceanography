...
# input us = 0*u here;
    ...
    out = np.array([tau,hsb*-1,hlb*-1,lw_net,sw_net])
...
if __name__ == '__main__':
    ...
    path = '/home/rohit/Downloads/'
    fil = 'test_data.txt'   
    #data = pd.read_csv(path+fil, header=0)
    data = np.genfromtxt(path+fil, skip_header=1,delimiter=",")
    u = data[:,1]
    t = data[:,3]
    rh = data[:,5]
    P = data[:,7]
    ...
    fnameA = os.path.join(path,'test_output.txt')
    ...
    A_hdr = 'tau\thsb\thlb\tlw_net\tsw_net'
